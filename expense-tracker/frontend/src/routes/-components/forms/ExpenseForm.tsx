...
import { expenseSchema } from "@/types/type";
...
import { useAddExpenseData, useUpdateExpenseByID } from "@/hooks/useExpense";
...
export default function ExpenseForm(props: any) {
  ...
  const { mutate: addExpenseData } = useAddExpenseData();
  const { mutate: updateExpenseData } = useUpdateExpenseByID();
  if (!isAddMode) {
  ...
  const form = useForm<z.infer<typeof expenseSchema>>(formOptions);
  ...
  const onSubmitForm = (data: z.infer<typeof expenseSchema>) => {
    form.reset();
    isAddMode ? createExpense(data) : updateExpense(data);
  };
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem>
                ...
                <FormControl>
                  <Input placeholder="Wai Wai Quick" {...field} />
                </FormControl>
